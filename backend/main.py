...
def karaoke_process(url):
    with process_lock:
        ...
        whisper_thread = threading.Thread(target=whisper_transcript, args=(uid, data_folder, whisper_q))
        whisper_thread.start()
        while True:
            try:
                whisper_progress = whisper_q.get(timeout=1)
                print("whisper_progress", whisper_progress)
                progress_data[uid] += whisper_progress/2 # whisper only occupies 50% of the total progress
