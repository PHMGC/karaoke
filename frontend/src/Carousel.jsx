...
const Carousel = () => {
    ...
    const [dbVideos, setDbVideos] = useState(9);
    ...
    useEffect(() => {
        const fetchCarouselData = async () => {
            try {
                ...
                setDbVideos(carouselData.length)
            } catch (error) {
    ...
    const sliderItems = dbVideos >= 10 ? testData : items;
        ...
        <div className="w-full mx-auto">
            ...
            <Slider {...settings}>
                {sliderItems.map((item) => (
                    <div onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={() => handleMouseUp(item.id)}
                     key={item.id} className="flex flex-col items-center p-4 hover:cursor-pointer outline-none">
                        <img src={item.image} alt={item.title} className="w-full h-auto object-cover rounded-md mb-2" />
                        <h2 className="text-black text-lg font-bold  text-center">{item.title}</h2>
                        <h3 className="text-gray-800 text-sm  text-center">{item.subTitle}</h3>
                    </div>
